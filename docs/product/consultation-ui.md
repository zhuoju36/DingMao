...
**检索本身是通的**（能拿到法条候选），但按 §3.2a 裁决，缺 `version`/`effective_date`
的引用必须丢弃 → 🟨 为空。`state_data.evidence_warnings` 里会记
`missing_law_version`，前端显示「知识库中 N 条引用因缺少版本号/生效日期被隐去」。

源数据 `knowledge-base/laws/dengcao-source/data/*.txt`（177 个文件）是**纯条文正文**，
不含公布/施行日期；`seed_knowledge.py` 只手工 seed 了几部。**没有可用的权威版本数据，
也不能编造（应用原则 3）。**

**解除条件**：从国家法律法规数据库导入这 177 部的 `version` + `effective_date`。
代码路径已就绪，导入后 🟨 自动生效，无需改代码。

### 8.5.1 检索术语表必须按语料分表（实测教训，已修）

**初版把一张合同法词汇表同时用于 `laws` 和 `standard_clauses`，这是设计错误。**

实测（3750 条强条语料）：20 个合同法术语命中 **0 次**——
...
原因：通用规范不谈价款/发包人/索赔，它谈混凝土/防水/消防/荷载。
**术语必须匹配语料的词汇体系，否则检索等于空转。**

修法：拆成 `LAW_TERMS`（合同法词汇）与 `STANDARD_TERMS`（工程技术词汇，
依据对 3750 条正文的词频统计策展），阈值分开设定并对"术语命中少"的案情自适应降阈。
...
且精度可核验：基坑→施工现场安全 3.5.2/3.5.3/3.5.4；混凝土裂缝→混凝土规范 2.0.8；
无障碍→无障碍规范 2.6.4；幕墙→建筑防火 6.2.4 + 民用建筑 6.2.8。
纯合同程序争议（无技术名词）正确返回 0 条强条。
